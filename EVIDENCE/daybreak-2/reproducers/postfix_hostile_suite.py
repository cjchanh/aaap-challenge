...
SOURCE_PACKET = ROOT / "reproducers" / "postfix-fixture-v2" / "packet"
SOURCE_ANCHOR_1 = ROOT / "reproducers" / "postfix-fixture-v2" / "anchor-1.json"
SOURCE_ANCHOR_2 = ROOT / "reproducers" / "postfix-fixture-v2" / "anchor-2.json"
SUITE = ROOT / "reproducers" / "hostile-suite-after-v3"
HEAD = "5305e32db27bed88c636fab8a141bd0beda94e70ec8ddd34ac513dcbf9bdbdad"
...
def fake_crypto_package(packet: Path, marker: Path, *, self_remove: bool) -> Path:
...
def main() -> int:
    ...
    manifest["key_model"] = "persistent-identity"
    ...
    text = (packet / "manifest.json").read_text(encoding="utf-8")
    text = text.replace('"key_model": "ephemeral-integrity"', '"key_model": NaN', 1)
    (packet / "manifest.json").write_text(text, encoding="utf-8")
    ...
    run_case(
        "anchor-overflow-number", "parser", packet, registry_args(first, second), 1, "PASS",
    ...
    case, packet = make_case("pre-validation-import-shadow")
    marker = case / "FORGERY_EXECUTED"
    package = fake_crypto_package(packet, marker, self_remove=True)
    manifest = json.loads((packet / "manifest.json").read_text(encoding="utf-8"))
    manifest["key_model"] = "persistent-identity"
    (packet / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    empty = case / "empty-path"
    ...
    env["PATH"] = str(empty)
    record = run_case(
        "pre-validation-import-shadow", "verifier-substitution", packet,
        explicit_args(), 1, "FAIL", env=env, classification="defense",
    )
    record["assertions"]["marker_absent"] = not marker.exists()
    record["assertions"]["unsealed_package_remained"] = package.exists()
    record["matched_expectation"] = record["matched_expectation"] and all(
        record["assertions"].values()
    )

    case, packet = make_case("stable-import-side-effect")
    marker = case / "STATIC_IMPORT_EXECUTED"
    package = fake_crypto_package(packet, marker, self_remove=False)
    record = run_case(
        "stable-import-side-effect", "verifier-substitution", packet,
        explicit_args(), 1, "FAIL", classification="defense",
    )
    record["assertions"]["marker_absent"] = not marker.exists()
    record["assertions"]["unsealed_package_remained"] = package.exists()
    record["matched_expectation"] = record["matched_expectation"] and all(
